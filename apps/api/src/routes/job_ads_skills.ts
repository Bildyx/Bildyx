...
import { JobAdSkillSchema, CreateJobAdSkillSchema, UpdateJobAdSkillSchema, GetJobAdSkillsSchema } from "../models/job_ads_skills";
import { z } from "zod";
...
export const job_ads_skills = {
  getAll: publicProcedure
    ...
    .handler(async ({ input }) => {
      ...
      if (job_ad_id) query = query.where('job_ad_id', '=', job_ad_id);
      if (skill_id) query = query.where('skill_id', '=', skill_id);
  ...
  getByJobAd: publicProcedure
    ...
    .handler(async ({ input }) => {
      ...
      if (!jobAd) throw new ORPCError("NOT_FOUND", { message: "Job ad not found" });
  ...
  create: publicProcedure
    ...
    .handler(async ({ input }) => {
      ...
      if (existing) throw new ORPCError("CONFLICT", { message: "This skill is already added to this job ad" });
      ...
      if (!job_ad_skill) throw new ORPCError("INTERNAL_SERVER_ERROR", { message: "Failed to add skill to job ad" });
  ...
  update: publicProcedure
    ...
    .input(z.object({ jobAdSkillId: z.string().uuid() }).merge(UpdateJobAdSkillSchema))
    .output(JobAdSkillSchema)
    .handler(async ({ input }) => {
      ...
      if (!existing) throw new ORPCError("NOT_FOUND", { message: "Job ad skill not found" });
      ...
      if (!job_ad_skill) throw new ORPCError("INTERNAL_SERVER_ERROR", { message: "Failed to update job ad skill" });
  ...
  delete: publicProcedure
    ...
    .handler(async ({ input }) => {
      ...
      await database.deleteFrom('job_ad_skills').where('id', '=', input.jobAdSkillId).execute();
