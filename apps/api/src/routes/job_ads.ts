...
import { JobAdSchema, CreateJobAdSchema, UpdateJobAdSchema, GetJobAdsSchema } from "../models/job_ads";
import { z } from "zod";
...
export const job_ads = {
  getAll: publicProcedure
    ...
    .handler(async ({ input }) => {
      const { search, organization_id, job_id, contract_type, remote, status, country_id, city_id } = input;

      let query = database.selectFrom('job_ads').where('deleted_at', 'is', null);
      ...
      if (job_id) query = query.where('job_id', '=', job_id);
      if (contract_type) query = query.where('contract_type', '=', contract_type);
      if (remote) query = query.where('remote', '=', remote);
  ...
  getOne: publicProcedure
    ...
    .handler(async ({ input }) => {
      ...
      if (!data) throw new ORPCError("NOT_FOUND", { message: "Job ad not found" });
  ...
  create: publicProcedure
    ...
    .handler(async ({ input }) => {
      ...
      if (!job_ad) throw new ORPCError("INTERNAL_SERVER_ERROR", { message: "Failed to create job ad" });
  ...
  update: publicProcedure
    ...
    .handler(async ({ input }) => {
      ...
      if (!existing) throw new ORPCError("NOT_FOUND", { message: "Job ad not found" });
      ...
      if (!job_ad) throw new ORPCError("INTERNAL_SERVER_ERROR", { message: "Failed to update job ad" });
  ...
  publish: publicProcedure
    ...
    .handler(async ({ input }) => {
      ...
      if (!job_ad) throw new ORPCError("INTERNAL_SERVER_ERROR", { message: "Failed to publish job ad" });
  ...
  close: publicProcedure
    ...
    .handler(async ({ input }) => {
      ...
      if (!job_ad) throw new ORPCError("INTERNAL_SERVER_ERROR", { message: "Failed to close job ad" });
  ...
  delete: publicProcedure
    ...
    .handler(async ({ input }) => {
      ...
      await database.updateTable('job_ads').set({ deleted_at: new Date() }).where('id', '=', input.jobAdId).execute();
