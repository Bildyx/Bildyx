...
type ScoreCard = { label: string; rawScore: number; maxScore: number; percentage: number };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const computedScores: Record<string, { rawScore: number; maxScore: number; percentage: number }> = {};

        criteriaDb.forEach((crit) => {
          const critQuestions = questionsDb.filter((q) => q.criterion_id === crit.id);
          let sum = 0;
          let count = 0;
        ...
        const cards: ScoreCard[] = criteriaDb.map((crit) => {
          const scoreInfo = computedScores[crit.code] || { rawScore: 0, maxScore: 50, percentage: 0 };
          const detailInfo = testDetail.criteria[crit.code] || { name: crit.name };
        ...
        <div className="res-shell">
          <section className="res-card" aria-labelledby="res-title">
            <header className="res-header">
              <Link className="res-back" to="/tests-preferences" aria-label="Back to tests and preferences">
                ‹
              </Link>
              <div>
                ...
                <p className="res-test-subtitle">{testDetail?.subtitle || ""}</p>
              </div>
            </header>
                ...
                {scoreCards?.map((card) => (
                  <div className="res-score-card" key={card.label}>
                    ...
                    <div className="res-card-progress-bar">
                      <div className="res-card-progress-bar-fill" style={{ width: `${card.percentage}%` }} />
                    </div>
                  </div>
                ))}
              </div>

              <div className="res-pdf-container" style={{ position: "relative" }}>
                {!pdfUrl && !error && (
                  <div className="res-pdf-loader">
                    <div className="res-spinner-container">
                      <div className="res-spinner" />
                      <p className="res-loader-text">Generating your report, please wait...</p>
                    </div>
                  </div>
                )}
                {error && <p style={{ padding: 24, color: "#dc2626" }}>{error}</p>}
                {pdfUrl && <iframe id="pdfViewer" src={pdfUrl} title="PDF Results" />}
              </div>
            </div>
          </section>
