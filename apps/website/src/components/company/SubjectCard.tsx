...
export function SubjectCard({ subjectId, onDelete }: { subjectId: string; onDelete?: () => void }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const cardHtml = await cardService.getSubject(subjectId);
        if (active) {
          setHtml(cardHtml);
        }
      } catch (err) {
  ...
  if (loading) {
    return (
      <div
        className="ca-photo-skeleton"
        style={{ height: "100%", minHeight: 260, borderRadius: 14 }}
      />
    );
  }
