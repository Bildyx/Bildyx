 ...
 *
 * Ported nearly verbatim from js/result.ts — this module is
 * framework-agnostic (pure DOM/Canvas/jsPDF), so it didn't need a React
 * rewrite, only relocation. The React-specific orchestration (fetching
 * scores, wiring the score cards, mounting the iframe) lives in
 * src/pages/ResultTest.tsx, which is the equivalent of the old IIFE at
 * the bottom of result.ts.
 *
 ...
 *
 * External libs required (loaded on demand via ensureLibs()):
 *   https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js
 *   jspdf is loaded the same way (see ResultTest.tsx's usePageMeta call
 *   which the page relies on to inject the jsPDF <script> tag, same as
 *   the original page's inline $jspdfScript).
 *
...
export const testDetailsMap: Record<string, TestDetail> = {
  BIG5: {
    name: "Big 5",
