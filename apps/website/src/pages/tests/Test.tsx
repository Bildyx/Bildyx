...
export default function Test() {
  usePageMeta(
    "Personality Test — Bildyx",
    "Take a personality test on Bildyx.",
  );
  ...
  const [questions, setQuestions] = useState<PersonalityQuestion[] | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!testDb) {
          console.error(
            "[Test] Test not found in database for code:",
            testCode,
          );
          return;
        }
            ...
            {isSubmitting && (
              <div className="pt-loader" style={{ display: "flex" }}>
                <div className="pt-spinner-container">
                  ...
                  <p className="pt-loader-text">
                    Saving answers and calculating results, please wait...
                  </p>
                </div>
              </div>
            )}
                ...
                <nav className="pt-question-list">
                  {questions === null
                    ...
                    : questions.map((q) => (
                        <a
                          key={q.id}
                          href={`#question-${q.order}`}
                          className={
                            answers[String(q.order)] ? "is-answered" : ""
                          }
                        >
                  ...
                  {questions === null
                    ? Array.from({ length: 3 }).map((_, i) => (
                        <div className="pt-skeleton-question" key={i}>
                          ...
                          />
                          <div
                            style={{ display: "flex", gap: 12, marginLeft: 42 }}
                          >
                    ...
                    : questions.map((q, idx) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {isYesNo ? (
                              <div
                                className="pt-scale pt-scale-yesno"
                                role="group"
                                aria-label={q.text}
                              >
                                <button
