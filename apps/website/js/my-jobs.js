document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderJob(jobId) {
        ...
        const group = activeCard ? activeCard.closest('[data-company-group]') : null;
    ...
    companyTabs.forEach((tab) => {
        tab.addEventListener('click', () => {
            ...
            const firstCard = document.querySelector(`[data-company-group="${company}"] .mj-job-card`);
            if (firstCard) renderJob(firstCard.dataset.job);
        });
    });

    document.addEventListener('click', (event) => {
        const applyButton = event.target.closest('[data-apply], [data-preview-apply]');
        if (applyButton) {
            event.stopPropagation();
            applyButton.classList.toggle('is-applied');
            applyButton.textContent = applyButton.classList.contains('is-applied') ? 'Applied' : 'Apply';
            return;
        }
