...
function renderProfile() {
  ...
  const points =
    currentProfileMode === "operate" ? team.operatePoints : team.overviewPoints;

  profilePoints.innerHTML = points
    .map(
      ([icon, title, description, danger]) => `
...
function renderTeam(teamKey) {
  ...
  products.forEach((product) => {
    product.classList.toggle(
      "active",
      product.dataset.product === team.activeProduct,
    );
  });
